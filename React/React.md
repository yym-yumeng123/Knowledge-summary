...
### 调和(Reconciliation)过程 和 Diff 算法

调和 又翻译为 协调:  虚拟DOM是一种编程概念, 在这个概念里, UI以一种理想化的, 或者说虚拟的表现形式被保存于内存中, 并通过如`ReactDOM等类库使之与真实的 DOM 同步, 这一过程叫做 协调

1. Diff 的设计思想

找两个树结构不同的过程: 传统通过循环递归进行树节点的一一对比

- 若两个组件属于同一个类型, 将拥有相同的DOM树形结构
- 处于同一层级的一组子节点, 可通过设置 key 作为唯一标识, 从而维持各个节点在不同渲染过程中的稳定性


### setState 是同步的还是异步的

- 只要在 React 管控下的 setState , 一定是异步的


1. 异步的动机和原理: 
- 批量更新: 每来一个 setState, 就把他塞进一个队列里"攒起来" 等时机成熟, 再把"攒起来"的state结果合并, 最后只针对最新的state值走一次更新流程

2. 同步现象: 从源码看 setState 工作流

- 在React钩子函数及合成事件中, 它表现为异步
- 在 setTimeout setInterval 等函数中, 包括在 DOM  原生事件, 他表现为同步

React16 以来, 整个 React 核心算法被重写, setState也不避免被`Fiber` 化

### React16 Fiber架构

`Stack Reconciler` 是一个同步的递归过程 => JS对主线程占用超时

Fiber? => 就是一个比线程还要纤细的过程, 也就是所谓的 "纤程", 出现意在对渲染过程实现更加惊喜的控制

从架构看: Fiber是对React核心算法的重写
从编码看: Fiber是React内部所定义的一种数据结构
从工作流: Fiber节点保存了组件需要更新的状态和副作用

1. Fiber 架构核心: 可中断 可恢复 优先级